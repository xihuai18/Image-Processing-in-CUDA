...
__global__ void CLAHEAft(int * hsi_img, int * g_frq, int height, int width)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        int tmp_idx = __umul24(tmp_y, width) + tmp_x;
        if(tmp_x < width && tmp_y < height) {
            int tmp = (1.0*g_frq[(hsi_img[tmp_idx]&0x0000FF)]/(height*width))*255;
            hsi_img[tmp_idx] = (hsi_img[tmp_idx] & 0xFFFF00) + ((255 < tmp)?255:tmp);
        ...
        if(tmp_x < width && tmp_y < height) {
            int tmp = (1.0*g_frq[(hsi_img[tmp_idx]&0x0000FF)]/(height*width))*255;
            hsi_img[tmp_idx] = (hsi_img[tmp_idx] & 0xFFFF00) + ((255 < tmp)?255:tmp);            // if ((1.0*frq[(i*3+2)*256+(hsi_img[tmp_idx]&0x0000FF)]/(TILESIZE*TILESIZE))*255 > 255) {
                // printf("==>ERROR!\n");
            // }
        }
    }
...
//            printf("%d: %d\n", i, g_frq[i]);
//        }
//        printf("aft: %d %d \n", hsi_img[thread_idx], g_frq[thread_idx]);
//        printf("powf(-1,2)=%f\n", powf(-1,2));
//    }
