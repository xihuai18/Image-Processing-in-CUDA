...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    for (int i = 0; i < input_img.height(); i++)
    {
        ...
        {
            input_array[j * input_img.height() + i] = (input_img.pixelColor(j, i).red() << 16) + (input_img.pixelColor(j, i).green() << 8) + (input_img.pixelColor(j, i).blue());
        }
    }
}
...
void MainWindow::on_pushButton_5_clicked()
{
    ...
    for (int j = 0; j < seed_img.width(); j++)
    {
        ...
        {
            seed_array[j * seed_img.height() + i] = (seed_img.pixelColor(j, i).red() << 16) + (seed_img.pixelColor(j, i).green() << 8) + seed_img.pixelColor(j, i).blue();
        }
    }
    ...
    endTime = std::clock();
    qDebug() << "CUT FINISH!"
             << "The run time is: " << (double)(endTime - startTime) / CLOCKS_PER_SEC << "s";
