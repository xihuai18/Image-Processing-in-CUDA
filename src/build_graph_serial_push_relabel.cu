...
void serialMaxflow(unsigned *res, int img_size, int col, int row, int bin_num, int *mask) {
  ...
  // push-relabel
  std::vector<int> h(n + 2, 0), h_count(2 * n + 2, 0), flow(n + 2, 0);
  h[S] = n;
  for (int i = last_edge[S]; i >= 0; i = edges[i].lh) {
    int q = edges[i].to;
    flow[q] = edges[i].f = edges[i].c;
    edges[i ^ 1].f -= flow[q];
  }
  bool finished;
  do {
    ...
    for (int p = 0; p < S; p++) {
      if (flow[p] > 0) {
        ...
        for (int i = last_edge[p]; i >= 0 && flow[p] > 0; i = edges[i].lh) {
          int q = edges[i].to;
          if (edges[i].f < edges[i].c) {
            if (h[p] == h[q] + 1) {
              min_h = -1;
              int f = min(flow[p], edges[i].c - edges[i].f);
              flow[p] -= f;
              if (q < S) flow[q] += f;
              edges[i].f += f;
              edges[i ^ 1].f -= f;
  ...
  //   }
  // }
  std::vector<int> queue(img_size + bin_num + 2, 0);
  std::vector<int> height(img_size + bin_num + 2, 1000000000);
  int head = 0, tail = 0;
  for (int i = last_edge[S]; i >= 0; i = edges[i].lh) {
    if (edges[i].f > 0) {
      height[edges[i].to] = 1;
  ...
  while (head < tail) {
    ...
    for (int i = last_edge[p]; i >= 0; i = edges[i].lh) {
      int q = edges[i].to;
